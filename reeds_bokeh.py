...
def reeds_static(data_type, data_source, scenario_filter, diff, base, static_presets, report_path, report_format, html_num, output_dir, auto_open):
    ...
    #First, add difference sections to static_presets if diff is 'Yes' and 'modify' is not already set for that preset
    if diff == 'Yes':
        i = 0
...
def get_wdg_reeds(path, init_load, wdg_config, wdg_defaults, custom_sorts, custom_colors):
    ...
    if scenarios:
        ...
        topwdg['report_diff'] = bmw.Select(title='Add Differences', value='No', options=['No','Yes'], css_classes=['report-drop'])
...
def build_reeds_report(html_num='one'):
    ...
    sp.call(start_str + ' "' + this_dir_path + '/reports/interface_report.py" ' + data_source + ' ' + scenario_filter_str + ' ' + diff + ' ' + base +' ' + report_path + ' "' + html_num + '" ' + output_dir + ' ' + auto_open + ' ' + data_type, shell=True)
